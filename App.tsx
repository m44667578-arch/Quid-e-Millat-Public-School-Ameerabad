...
import { Page, User, AuthModalState, UserType, AuthMode, SchoolEvent, Notice, ResultSheet, Complaint, Grade, GalleryItem, AdmissionApplication, LeadershipMessage } from './types';
...
const App: React.FC = () => {
  ...
  const renderPage = () => {
    switch (currentPage) {
      case Page.Home:
        return <Home studentCount={studentCount} staffCount={staffCount} openAuthModal={openAuthModal} registrationMessage={registrationMessage} leadershipMessages={leadershipMessages}/>;
      case Page.About:
        return <About />;
      ...
      case Page.PrincipalDashboard:
        return loggedInUser?.userType === 'principal' ? (
          ...
          />
        ) : <Home studentCount={studentCount} staffCount={staffCount} openAuthModal={openAuthModal} registrationMessage={registrationMessage} leadershipMessages={leadershipMessages} />;
      case Page.StudentDashboard:
        return loggedInUser?.userType === 'student' && loggedInUser.studentId ? <StudentDashboard user={loggedInUser} resultSheets={resultSheets} complaints={complaints.filter(c => normalizeId(c.studentId) === normalizeId(loggedInUser.studentId))} /> : <Home studentCount={studentCount} staffCount={staffCount} openAuthModal={openAuthModal} registrationMessage={registrationMessage} leadershipMessages={leadershipMessages} />;
      case Page.ParentDashboard:
        return loggedInUser?.userType === 'parent' ? <ParentDashboard user={loggedInUser} allResultSheets={resultSheets} allComplaints={complaints} allApprovedUsers={approvedUsers} /> : <Home studentCount={studentCount} staffCount={staffCount} openAuthModal={openAuthModal} registrationMessage={registrationMessage} leadershipMessages={leadershipMessages} />;
      case Page.TeacherDashboard:
        return loggedInUser?.userType === 'admin' ? <TeacherDashboard user={loggedInUser} notices={notices} events={events} onAddComplaint={handleAddComplaint} approvedStudents={approvedUsers.filter(u => u.userType === 'student')} onSaveEvent={handleSaveEvent} onPostNotice={handlePostNotice} onAddGalleryItem={handleAddGalleryItem} /> : <Home studentCount={studentCount} staffCount={staffCount} openAuthModal={openAuthModal} registrationMessage={registrationMessage} leadershipMessages={leadershipMessages} />;
      default:
        return <Home studentCount={studentCount} staffCount={staffCount} openAuthModal={openAuthModal} registrationMessage={registrationMessage} leadershipMessages={leadershipMessages} />;
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans text-gray-800">
      <Header navigate={navigate} user={loggedInUser} onLogout={handleLogout} />
